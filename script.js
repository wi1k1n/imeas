(function() {
    ...
    function zoom(evt) {
        ...
        imgOffset.x = evt.x - (evt.x - imgOffset.x) * mlt;
        imgOffset.y = evt.y - (evt.y - imgOffset.y) * mlt;

        redrawImage();
    }

    function redrawImage() {
        let sx = -Math.min(imgOffset.x / imgScale.x, 0);
        let sy = -Math.min(imgOffset.y / imgScale.y, 0);
        
        let sw = Math.min((w - imgOffset.x) / imgScale.x, img.width) - sx;
        let sh = Math.min((h - imgOffset.y) / imgScale.y, img.height) - sy;
        let dx = Math.max(imgOffset.x, 0);
        let dy = Math.max(imgOffset.y, 0);
        let dw = sw * imgScale.x;
        let dh = sh * imgScale.y;
        // let s = 'sx: ' + sx + '<br>';
        // s += 'sx: ' + sx + '<br>';
        // s += 'sy: ' + sy + '<br>';
        // s += 'sw: ' + sw + '<br>';
    ...
    (function initializeHotKeys() {
        ...
        hotkeys('ctrl+o,esc,space', function (event, handler){
            switch (handler.key) {
                case 'ctrl+o':
    ...
    if (DEBUG) dbg.classList.remove('hidden');
    function debug(v) {
        if (!DEBUG) return;
